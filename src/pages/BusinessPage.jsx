...
function BusinessPage({ eventBusiness }) {
  const [filteredEvents, setFilteredEvents] = useState(eventBusiness);
  ...
  function handleSearch(data) {
    setSearchInput(data);
    const filteredEvents = eventBusiness.filter(
      (event) =>
        event.event_name &&
  ...
  function handleFilter(data) {
    ...
    let updatedEvents = eventBusiness.slice(); // Copy the original event data
    ...
    <div>
      ...
      <div className="px-6 m-4 space-y-4">
        {filteredEvents.length > 0 ? (
          ...
          <p>No events match the current search in business category.</p>
        )}
      </div>
    </div>
  );
