...
function MbtiExtrovert({ eventSports }) {
  const [filteredEvents, setFilteredEvents] = useState(eventSports);
  ...
  function handleSearch(data) {
    setSearchInput(data);
    const filteredEvents = eventSports.filter(
      (event) =>
        event.event_name &&
  ...
  function handleFilter(data) {
    ...
    let updatedEvents = eventSports.slice(); // Copy the original event data
    ...
    <div>
      ...
      <div className="px-6 m-4 space-y-4">
        {filteredEvents.length > 0 ? (
          ...
          <p>No events match the current search in sports category.</p>
        )}
      </div>
    </div>
  );
