...
function MbtiIntrovert({ eventmbti }) {
  const updataEventmbti = eventmbti.extrovert;
  const [filteredEvents, setFilteredEvents] = useState(eventmbti.extrovert);
  ...
  function handleSearch(data) {
    setSearchInput(data);
    const filteredEvents = filteredEvents.filter(
      (event) =>
        event.event_name &&
    ...
    <div>
      ...
      <div className="px-6 m-4 space-y-4">
        {filteredEvents.length > 0 ? (
          ...
          <p>No events match the current search in MBTI-Extrovert category.</p>
        )}
      </div>
    </div>
  );
