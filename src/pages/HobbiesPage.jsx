...
function HobbiesPage({ eventHobbies }) {
  const [filteredEvents, setFilteredEvents] = useState(eventHobbies);
  ...
  function handleSearch(data) {
    setSearchInput(data);
    const filteredEvents = eventHobbies.filter(
      (event) =>
        event.event_name &&
  ...
  function handleFilter(data) {
    ...
    let updatedEvents = eventHobbies.slice(); // Copy the original event data
    ...
    <div>
      ...
      <div className="px-6 m-4 space-y-4">
        {filteredEvents.length > 0 ? (
          ...
          <p>No events match the current search in hobbies category.</p>
        )}
      </div>
    </div>
  );
