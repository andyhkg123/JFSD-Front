...
function BoardgamesPage({ eventBoardgames }) {
  const [filteredEvents, setFilteredEvents] = useState(eventBoardgames);
  ...
  function handleSearch(data) {
    setSearchInput(data);
    const filteredEvents = eventBoardgames.filter(
      (event) =>
        event.event_name &&
  ...
  function handleFilter(data) {
    ...
    let updatedEvents = eventBoardgames.slice(); // Copy the original event data
    ...
    <div>
      ...
      <div className="px-6 m-4 space-y-4">
        {filteredEvents.length > 0 ? (
          ...
          <p>No events match the current search in board games category.</p>
        )}
      </div>
    </div>
  );
