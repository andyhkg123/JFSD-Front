...
function MbtiIntrovert({ eventmbti }) {
  const updataEventmbti = eventmbti.introvert;
  const [filteredEvents, setFilteredEvents] = useState(eventmbti.introvert);
  ...
  function handleSearch(data) {
    setSearchInput(data);
    const filteredEvents = updataEventmbti.filter(
      (event) =>
        event.event_name &&
  ...
  function handleFilter(data) {
    ...
    let updatedEvents = updataEventmbti.slice(); // Copy the original event data
    ...
    <div>
      ...
      <div className="flex flex-row flex-wrap space-x-2 mx-4 pt-2 px-6">
        <Link to="/mbti_introvert">
          <button className="inline-flex justify-center items-center py-2 px-5 text-sm font-medium text-center text-gray-600 rounded-lg border-grey-200 border bg-amber-100 hover:bg-gray-50">
            Introvert
          </button>
        </Link>
      ...
      <div className="px-6 m-4 space-y-4">
        {filteredEvents.length > 0 ? (
          ...
          <p>No events match the current search in MBTI-Introvert category.</p>
        )}
      </div>
    </div>
  );
